...
<div class="grid md:grid-cols-2 my-8">
  <div class="border-r border-neutral-light pr-6">
    <div class="flex items-center justify-center gap-2 mb-6">
      ...
      <Heading level={4} as="h4" class="text-base font-semibold text-text">Before: Sequential Collection</Heading>
    </div>
    <figure>
      <MermaidDiagram
        id="workflow-before"
        caption="8-10h total: 2h collection (4 man-hours), then 6-8h post-processing (validation, conflicts, freeze frames)"
        relationships={[
          "Collection: 2 collectors × 2h = 4 man-hours",
          "Post-processing: 6-8h for validation, conflict resolution, freeze frames",
          "Total: 8-10h wall-clock = 16-20 man-hours",
          "Sequential dependency creates bottleneck"
        ]}
      >

```mermaid
graph TD
    Start["Match begins"] --> Collect["Collection phase
    2 collectors × 2h
    4 man-hours"]
    Collect --> Post["Post-processing
    6-8 hours"]
    Post --> Valid["Validation +
    Conflict resolution +
    Freeze frames"]
    Valid --> Complete["Match complete
    8-10h total
    16-20 man-hours"]

    classDef problem fill:#FEF3C7,stroke:#F59E0B,stroke-width:2px,color:#78350F
    class Start,Collect,Post,Valid,Complete problem
```
      ...
      <details class="mt-2 text-xs text-text-lighter text-center">
        ...
        <p class="mt-1">Sequential workflow: 2 collectors work 2h in parallel for collection (4 man-hours), then 6-8h post-processing for validation, conflict resolution, and freeze frames. Total: 8-10h wall-clock = 16-20 man-hours.</p>
      </details>
    </figure>
  </div>
  ...
  <div class="pl-6">
    <div class="flex items-center justify-center gap-2 mb-6">
      ...
      <Heading level={4} as="h4" class="text-base font-semibold text-text">After: Concurrent Collection</Heading>
    </div>
    <figure>
      <MermaidDiagram
        id="workflow-after"
...
```mermaid
graph TD
    Start["Match begins"] --> Base["Base event creation
    (name, timestamp)"]
    Base --> E1["Extras enrichment"]
    Base --> E2["Location enrichment"]
    Base --> E3["FreezeFrame enrichment"]
    E1 --> M["Match ends
    ~2 hours"]
    E2 --> M
    E3 --> M
...
```
      ...
      <details class="mt-2 text-xs text-text-lighter text-center">
        <summary class="cursor-pointer hover:text-text">Text alternative</summary>
        <p class="mt-1">Concurrent workflow: Base events (name, timestamp) created during match with 3 parallel enrichments (Extras, Location, FreezeFrame), followed by 5-minute automated merge at match end. Total: ~2 hours.</p>
      </details>
    </figure>
  </div>
</div>

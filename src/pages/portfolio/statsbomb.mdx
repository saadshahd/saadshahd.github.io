...
```
        ...
        <Body size="sm" as="p" class="mb-6 text-neutral italic">
          The lead collector (amber) establishes event identity. GraphQL subscriptions (blue) broadcast via WebSocket.
          Concurrent collectors (amber) contribute specialized data to distinct fields. The merge service (emerald)
          combines contributions without conflicts—role separation ensures each collector owns unique data dimensions.
        </Body>

This tool became the foundation. It proved that architecture as data worked in practice: the dataspec drove UI validation, the DSL defined legal event sequences, the state machines enforced correctness. 16 sequential hours → 4 man-hours during the live match with near real-time (~20s latency). The backend scaled to support what the UX tool proved collectors needed.
    </div>
...
Week one was me and Ali hand-writing rules. Month one backend: a single Go endpoint called `sync`—batch, offline. I started building the live-collection-app with an aircraft engineer friend who was shifting careers to software. No team. No process. Just the urgent need to replace Dartfish.
...
The tension: I didn't want to compromise on the innovative ideas the domain and scale enforced us to explore. But innovation without transmission meant I became the bottleneck. Every new context, every subsystem expansion, every architectural decision required me.
...
They took the foundational concepts and applied them to contexts I hadn't reached yet.
...
Invest in team building from day one, not year three. The two-year delay meant racing geo-political constraints before completing the customer-facing DSL. Adham, Hadeel, Waheed, and Abdallah iterated beyond my original vision—they should have started sooner.

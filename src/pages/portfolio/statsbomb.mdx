...
<Container width="narrow" spacing="default">
  ...
  <Section spacing="spacious">
    ...
    {/* Core lesson callout */}
    <Callout variant="primary" size="compact" icon="lightbulb" title="Core Lesson">
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const marker = depthMarkers.find(
              (m) => entry.target.matches(m.selector)
            );

            if (marker && !firedDepths.has(marker.depth)) {
              // Track this depth milestone
              trackScrollDepth('statsbomb-case-study', marker.depth, marker.label);

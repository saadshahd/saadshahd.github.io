...
<EditorialNote>
  ...
  architectural decisions enable scale and multi-sport expansion through
...
The dataspec itself had to be uncertain. From week one, we knew: not _if_ we'd iterate, but _how often_. Our historical data needed to evolve with our understanding—not versioned snapshots frozen in time, but living data that grew smarter as we did.
...
{/* Scroll depth tracking: Handled by global script in Layout.astro */}

...
```mermaid
graph TD
    Start["Match begins"] --> HomeCollector["Home collector"]
    Start --> AwayCollector["Away collector"]
    HomeCollector --> Merge1["Reconcile"]
    AwayCollector --> Merge1
    Merge1 --> HomePost["Validation + Frames"]
    Merge1 --> AwayPost["Validation + Frames"]
    HomePost --> Merge2["Final merge"]
    AwayPost --> Merge2
    Merge2 --> Complete["Match complete
    ~12h total
    ~20 man-hours"]

    classDef problem fill:#FEF3C7,stroke:#F59E0B,stroke-width:2px,color:#78350F
    class Start,HomeCollector,AwayCollector,Merge1,HomePost,AwayPost,Merge2,Complete problem
```
      ...
      <details class="mt-2 text-xs text-text-lighter text-center">
        ...
        <p class="mt-1">Before workflow: Home and Away collectors work in parallel (4h each). First merge reconciles collection conflicts (1h). Then parallel post-processing for validation, conflicts, and freeze frames (4h each). Final merge reconciles post-processing conflicts (1h). Total: ~12h wall-clock = 20 man-hours.</p>
      </details>
    </figure>
  </div>
...
graph TD
    ...
    class Start,Base,E1,E2,E3,M,N,O success
```

      </MermaidDiagram>
      <details class="mt-2 text-xs text-text-lighter text-center">
        <summary class="cursor-pointer hover:text-text">Text alternative</summary>
        <p class="mt-1">Concurrent workflow: Base events (name, timestamp) created during match with 3 parallel enrichments (Extras, Location, FreezeFrame), automated merge, complete layered analysis at match end. Total: ~2 hours wall-clock = ~7 man-hours.</p>
      </details>
    </figure>
  </div>
</div>
...
<p class="mb-6 text-text-lighter italic">
  Freeze frame collection reduced from 12 minutes (manual clicking, Dartfish) to 30 seconds (CV-assisted workflow). Computer vision detected 90% automatically; collectors corrected edge cases with eyes on video, hands on keyboard. — A.Magdy, 360 Collection Lead
</p>

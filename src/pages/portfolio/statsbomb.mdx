...
<div class="max-w-4xl mx-auto px-4 md:px-6 py-20">
  ...
  <section class="mb-12">
    ...
    {/* Reading metadata */}
    <ReadingMetadata readingTime="26 min" sectionCount={10} class="mb-8" />
    ...
    <details class="mb-8 p-6 bg-surface rounded-lg border border-neutral-light" open>
      ...
      <ul class="space-y-3 mb-6 text-text-lighter">
        ...
        <li><strong>Business Impact:</strong> Product managers shipped new sports independently, 5-person metadata team scaled to collectors, automated validation prevented errors before they happened</li>
      </ul>
...
<p class="text-center">`start → [middle] → break`</p>
...
<div class="grid md:grid-cols-2 gap-4 my-6 max-w-[696px] mx-auto">
  <figure>
    <img
      ...
      loading="lazy"
    />
    <figcaption class="text-xs text-text-lighter mt-1 text-center italic">Viewport calculations & shot extrapolation</figcaption>
  </figure>
  ...
  <figure>
    ...
    <p class="text-xs text-text-light text-center italic -mt-2 mb-6">Week One sketches (2018): From these rough drawings to production tool in days</p>
  </section>
...
<div class="grid md:grid-cols-2 my-8">
  <div>
    <div class="flex items-center justify-center gap-2 p-2 border border-r-0 border-neutral-light">
...
Waheed built claims-based metadata resolution—the automated conflict detection system we'd only sketched conceptually. He took provenance + confidence + temporality and made it production-ready, enabling 5 people to resolve conflicts for thousands collectors.

Hadeel implemented real-time GraphQL subscriptions—the concurrent collection we'd prototyped but hadn't hardened for collectors working simultaneously on live matches.
...
<Callout variant="primary" icon="lightbulb" title="The Team Building Lesson">
  <p>Architecture shapes what's possible. But people make it real.</p>
</Callout>
  </section>
...
<Quote icon={true} attribution="A.Magdy, 360 Collection Lead">
  With Dartfish, 12 minutes just to freeze a single shot. Now we freeze every event in the match—passes, tackles, everything. The tool made this scope possible.
</Quote>

<Callout variant="primary" icon="lightbulb" title="The 80/20 Lesson" class="mb-6">
  <Body size="sm">Age 14. Nasr City to downtown on a Cairo bus, reading "The Pragmatic Programmer"
    on my phone. Hunt & Thomas: "Design for the 80% case." That paragraph stuck.
    Sixteen years later, every architectural decision traced back—DSL optimized for
    standard match sequences, state machines for common collector flows, metadata
    for typical events. When 95% of matches follow patterns, build for patterns.
    Production revealed the remaining 5%.</Body>
</Callout>

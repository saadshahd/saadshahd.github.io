  ...
  <section class="mb-12">
    ...
    <details class="mb-8 p-6 bg-surface rounded-lg border border-neutral-light" open>
      ...
      <Callout variant="primary" icon="lightbulb" title="Core Lesson">
        <p>Architectural separation multiplied team leverage—product managers shipped new sports without engineering bottlenecks. When Adham, Waheed, Hadeel, and Abdallah took ownership, they evolved the system beyond what we started with.</p>
      </Callout>
    </details>
  </section>
...
That's when we'd debug together: Adham tracing the Kafka lag, me testing state machine transitions with a tired collector who just wanted to finish the match and go home.
  ...
  <section class="mb-8" id="architecture">
    <Heading level={3} as="h2" class="mb-6 text-navy">Architecture: Three Systems in Parallel</Heading>
...
Remember week one—Ali dictating rules while I typed in notepad? That's when it clicked: the dataspec can't be code. It has to be data we can iterate without deploys. That became the DSL principle.

Product managers needed to express domain logic—entry validation, sequencing logic, aggregation rules—in readable syntax that compiled to execution logic. Rules as data, not code.
...
Remember the 2am Test? World Cup extra time, keyboard glitches, WebSocket drops. Those failures taught us: real-time isn't a feature checkbox. When collectors are exhausted at 2am during penalty shootouts, the tool can't allow ambiguous inputs. Prevention through state machines, not validation after the fact—that became the XState requirement.
...
**Why Dartfish Couldn't Scale**: The industry standard forced button-heavy mouse clicks for every selection. It had keyboard shortcuts, but each key mapped to only one event type globally. With 50+ event types but only 40 usable keys, collectors memorized arbitrary mappings regardless of game context. Cognitive overload blocked velocity.

**State Machines for Prevention**: We modeled collection workflows as explicit finite state machines with XState. Define workflows as explicit states with clear transitions, and illegal states become structurally impossible. Can't skip steps. Can't submit incomplete data.
...
The dataspec drove UI validation, DSLs defined legal sequences, state machines constrained invalid states. Architecture caught errors before collectors saw them.
    </Accordion>
...
**Backend Evolution**: Month one: Omar Negm built the single Go endpoint called `sync`—batch, offline. In year two (2019), when Omar left to teach kids software, Adham joined and designed the evolution to Kafka as persistence center, event logs as foundation—enabling real-time collection at broadcaster scale.
  ...
  <section class="mb-8" id="dataspec-paradox">
    <Heading level={3} as="h2" class="mb-6 text-navy text-center max-w-[600px] mx-auto">The Dataspec Velocity Tradeoff: How Do You Change Rules Without Invalidating History?</Heading>
...
The dataspec itself had to be uncertain. By week one, the question shifted: not *if* we'd iterate, but *how often*. Our historical data needed to evolve with our understanding—not versioned snapshots frozen in time, but living data that grew smarter as we did.
  </section>
  ...
  <section class="mb-8" id="team-building">
    <Heading level={3} as="h2" class="mb-6 text-navy ">Distributed Ownership: Learning to Scale</Heading>
...
**Adham** advocated for Kafka in year two. My reaction: this feels complex for what we're doing. His counter: the domain will grow into it. He was right. By year three, event streaming wasn't "advanced tooling"—it was how we handled parallel collection and historical corrections at scale. What seemed like over-engineering became structural necessity as domain complexity revealed itself. That moment taught me distributed ownership meant trusting teammates to see architectural needs I couldn't.
...
The patterns Ali and I sketched in notepad during week one became valuable enough for an industry leader to buy. The architecture that emerged from serving collectors directly, from watching what slowed them down and making it faster, had legs beyond what we imagined.
  ...
  {/* Transition Bridge to Technical Sections */}
  <Body size="sm" as="p" class="mb-12 text-text-lighter italic text-center max-w-[700px] mx-auto">The acquisition validated the collection system—but what made 5000+ collectors possible? Three architectural decisions: configuration-driven domain logic, prevention through state machines, and event graphs for time. Here's the quantified impact, then how we built it.</Body>
...
<div class="grid md:grid-cols-2 gap-6 mb-8">
  <ComparisonCard variant="before" title="Engineering Bottleneck">
    <ul class="space-y-3">
      <li><strong>American football expansion:</strong> 12-month engineering estimate blocked product roadmap and market entry timing</li>
      <li><strong>Dataspec changes:</strong> 5-day engineering cycle (code → test → deploy → regression) per rule change</li>
      <li><strong>Support burden:</strong> 2 product managers fielded 50+ daily Slack questions about rules and edge cases</li>
    </ul>
  </ComparisonCard>

  <ComparisonCard variant="after" title="Product Manager Independence">
    <ul class="space-y-3">
      <li><strong>NFL shipped day one:</strong> Product managers wrote drive segmentation rules independently, zero engineering bottleneck</li>
      <li><strong>Dataspec changes:</strong> 5-minute product manager deploy via DSL editor (we shipped twice daily during World Cup 2022)</li>
      <li><strong>Automated resolution:</strong> 5-person metadata team handled 2% escalations while system resolved 98% automatically</li>
    </ul>
  </ComparisonCard>
</div>
...
<Body size="sm" as="p" class="mb-6 text-text-lighter italic">From 100 to 5000+ collectors, product managers shipped new dataspecs without proportional engineering investment, though complex state transitions occasionally required technical collaboration.</Body>
  </section>
